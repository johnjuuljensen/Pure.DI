...
internal class BindingMetadata : IBindingMetadata
{
    ...
    public override string ToString()
    {
        ...
        foreach (var dependency in _dependencies)
        {
            ...
            sb.Append($"{nameof(IBinding<Unit>.Bind)}<{dependency}>({tags}).");
        }

        if (Lifetime != Lifetime.Transient)
        {
            sb.Append($"{nameof(IBinding<Unit>.As)}({Lifetime}).");
        }

        foreach (var tag in _tags)
        {
            sb.Append($"{nameof(IBinding<Unit>.Tags)}<{tag.ToString()}>().");
        }

        sb.Append(Factory != null ? $"{nameof(IBinding<Unit>.To)}({Factory.ToString()})" : $"{nameof(IBinding<Unit>.To)}<{Implementation}>()");
        return sb.ToString();
    }
}