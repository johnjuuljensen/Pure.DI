...
internal class FactoryRewriter : CSharpSyntaxRewriter
{
    ...
    public override SyntaxNode VisitGenericName(GenericNameSyntax node)
    {
        ...
        if (node.Identifier.Text == nameof(IContext<Unit>.Resolve) && node.Parent?.Parent is InvocationExpressionSyntax invocation && IsResolveMethod(invocation))
        {
    ...
    private bool IsResolveMethod(InvocationExpressionSyntax invocation)
    {
        ...
        if (memberAccess.Expression is IdentifierNameSyntax identifierName && identifierName.ToString() == _contextIdentifier.Text)
        {
            ...
            if (genericName.Identifier.Text != nameof(IContext<Unit>.Resolve)) return false;
